...
The next insight that we found is we found how peoples happiness changed before and after Covid-19 another thing we found intruiging is how countries GPT per capita affects how many covid cases there are. We can extract how a nation's corruption level affects its GDP per capita and hence its covid cases (provided they are related). 
We explored many aspects of the data, but will demonstrate three. These are that gdp per capita increases covid cases tend to increase as well, European countries tend to generate the most capital while simultanouesly being the least corrupt and that covid affected Asia and Europe the most. 

1. GDP per capita focuses on the proportion of economic output per person. GDP acts as a measure of economic propersity of a country and therefore an increase in population over time. This increase of population leads to a larger spread of covid and hence more people contracting the virus. So, as the x varible of GDP per capita increases, the y axis increases too, signifying a positive correlation coefficient.
...
Finally, \@ref(fig:insight3) shows the relationship between the positive affect and the total covid cases per capita for every continent. This analysis was performed by using the column covid per capita and positive affect.
