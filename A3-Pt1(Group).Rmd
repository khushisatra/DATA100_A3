...
left_join(aware_pop, by = "country")
...
aware_covid <- covid_2020_match_country|>
  inner_join(aware_pop, by = c("country"))|>
...
# Joining The Data (Ideas)

```{r}
ice_extent_yearly_md <- ice_extent_daily |>
  group_by(year, region) |>
  summarise(
  min = min(ice_extent),
  max = max(ice_extent),
  ...
  ice_extent_yearly_md

merged_data <- climate_awareness|>
  inner_join(happiness, by = "country") |>
  inner_join(ice_extent_yearly_md, by = "year")

merged_data


ggplot(merged_data, aes(x = min, y = aware_no)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  labs(title = "Min Ice Extent vs Climate Awareness", x = "Min Ice Extent", y = "Climate Awareness (No)")

ggplot(merged_data, aes(x = max, y = life_ladder)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  labs(title = "Max Ice Extent vs Happiness", x = "Max Ice Extent", y = "Average Life Ladder (Happiness)")
```

```{r}

ggplot(merged_data, aes(x = year)) +
  geom_line(aes(y = min, color = "Min Ice Extent"), size = 1) +
  geom_line(aes(y = max, color = "Max Ice Extent"), size = 1) +
  scale_color_manual(values = c("blue", "red")) +
  labs(
    title = "Sea Ice Extent Over Time",
    x = "Year", y = "Sea Ice Extent (Million km²)",
    color = "Ice Extent Type"
  ) +
  theme_minimal()
```
```{r}
#2023
happiness_cad <- happiness |>
  filter(
...
#2020 in COVID, 2022 in awareness 
aware_covid_cad <- aware_covid |>
  filter(
    country == 'Canada'
  ) |>
  select (country,aware_no, aware_alittle, aware_moderate, aware_alot, aware_refuse, covid_per_capita)
aware_covid_cad
